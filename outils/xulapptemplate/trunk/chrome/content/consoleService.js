...
//====================================================
// Utilities To log messages in the JSconsole
//====================================================

var gJSConsoleService = Components.classes["@mozilla.org/consoleservice;1"].
  getService(Components.interfaces.nsIConsoleService);


function logMessage(aMessage) { 
  gJSConsoleService.logStringMessage(aMessage);
}

function logError(aMessage,aSourceName,aSourceLine)
{
  var errorObject = Components.classes['@mozilla.org/scripterror;1'].createInstance(Components.interfaces.nsIScriptError);
  if(errorObject) {
    errorObject.init(aMessage, aSourceName, aSourceLine, 0, 0, 0, "3LIZ");
    gJSConsoleService.logMessage(errorObject);
  }
}

function logWarning(aMessage, aSourceName, aSourceLine, aFlag)
{
  var errorObject = Components.classes['@mozilla.org/scripterror;1'].createInstance(Components.interfaces.nsIScriptError);
  if (gConsoleService && errorObject) {
    errorObject.init(aMessage, aSourceName, aSourceLine, 0, 0, 1, "3LIZ");
    gJSConsoleService.logMessage(errorObject);
  }
}
